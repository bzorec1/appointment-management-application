...
public sealed class AppointmentEndpointDefinition : IEndpointDefinition
{
    ...
    public void DefineEndpoints(RouteGroupBuilder api)
    {
        ...
        group.MapPost("/", async (
                NewAppointment dto,
                ISchedulingService sched,
                CancellationToken cancellationToken) =>
            {
                try
                {
                    var saved = await sched.CreateAsync(dto, cancellationToken);
                ...
                catch (InvalidOperationException)
                {
                    ...
                    {
                        message = "Termin se prekriva za izbranega stilista."
                    });
                }
            })
        ...
        group.MapGet("/", async (
                DateTimeOffset from,
                DateTimeOffset to,
                IAppointmentDataStore dataStore,
                CancellationToken cancellationToken) =>
            {
                if (to <= from)
                {
                ...
                var list = await dataStore.ListAsync(from, to, cancellationToken);
        ...
        group.MapGet("/{id:int}", async (
                ...
                CancellationToken cancellationToken) =>
            {
                var appointment = await dataStore.GetByIdAsync(id, cancellationToken);
        ...
        group.MapPut("/{id:int}", async (
                ...
                CancellationToken cancellationToken) =>
            {
                try
                {
                    var updated = await sched.UpdateAsync(id, dto, cancellationToken);
        ...
        group.MapDelete("/{id:int}", async (
                ...
                CancellationToken cancellationToken) =>
            {
                var deleted = await sched.DeleteAsync(id, cancellationToken);
                return deleted
                    ? Results.NoContent()
                    : Results.NotFound();
            })
        ...
        group.MapGet("/{id:int}/calendar.ics", async (
                int id,
                IAppointmentDataStore dataStore,
                CancellationToken cancellationToken) =>
            {
                var appt = await dataStore.GetByIdAsync(id, cancellationToken);
                if (appt is null)
                {
    ...
    static string BuildIcs(AppointmentDto dto)
    {
        var dtStamp = DateTime.UtcNow.ToString("yyyyMMddTHHmmssZ");
        var dtStart = dto.Start.UtcDateTime.ToString("yyyyMMddTHHmmssZ");
        var dtEnd = dto.End.UtcDateTime.ToString("yyyyMMddTHHmmssZ");
        var description = $"Storitev: {dto.Service}\\nStranka: {dto.CustomerName}\\nTel: {dto.Phone}";
