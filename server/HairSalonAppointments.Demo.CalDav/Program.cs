...
for (int run = 1; run <= iterations; run++)
{
    ...
    sw.Stop();
    CsvLog.Append("caldav", "caldav", "create", sw.ElapsedMilliseconds, (int)create.StatusCode, create.IsSuccessStatusCode,
        $"run_{run}");
    ...
    sw.Stop();
    CsvLog.Append("caldav", "caldav", "list", sw.ElapsedMilliseconds, (int)list.StatusCode, list.IsSuccessStatusCode,
        $"run_{run}");
