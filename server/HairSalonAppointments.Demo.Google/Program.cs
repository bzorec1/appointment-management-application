...
for (int run = 1; run <= iterations; run++)
{
    ...
    sw.Stop();
    CsvLog.Append("google", "google", "create", sw.ElapsedMilliseconds, (int)create.StatusCode, create.IsSuccessStatusCode,
        $"run_{run}");
    ...
    sw.Stop();
    CsvLog.Append("google", "google", "list", sw.ElapsedMilliseconds, (int)list.StatusCode, list.IsSuccessStatusCode,
        $"run_{run}");
